...
#' @param layer character(1). The Seurat object assay layer to use (Default:"data").
...
RunMDS <- function(object, distance=NA, n.dims = 30, features.use = NA, assay=NA, layer="data", reduction.name="mds", key="MDS_", batch_size=400, n.cores=1, seed = 42){
  set.seed(seed)
  ...
  mds <- bigmds::divide_conquer_mds(x = Matrix::t(GetAssayData(object, assay = assay, layer = layer)[features.use,]), l = batch_size, c_points = 2*n.dims, r = n.dims, n_cores = n.cores, dist_fn = distance)
