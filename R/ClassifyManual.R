...
#'
#' The signatures can also come from other sources, and simply need to be present either as metadata columns or as features in any assay. In case they are features, the data slot is used.
#'
...
#' @param slot If some signatures used for classification are stored as assay features, which slot should be used (Default: "data").
...
ClassifyManual <- function(object, gates, metadata.name="celltype", slot="data"){
  # Split by cell types:
  gates <- gsub(" ", "", gates)
  gates <- gsub("\n", "", gates)
  ...
  df <- GatherFeatures(object, unique.signatures.needed)
