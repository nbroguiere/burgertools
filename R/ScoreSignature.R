...
#' By default, the score is computed as the average expression of the features as normalized in the "data" layer (i.e. log-normalized typically). Can be overrun by setting the layer.use parameter. 
...
#' @param layer.use The layer to use within the assay to get data for signature scoring. By default, using the "data" assay.
#' @return Seurat object with an additional metadata column containing the signature score.
...
#' @export
#' 
ScoreSignature <- function(object, features, name = "Signature", assay.use=NA, layer.use="data"){
  ...
  features.positive <- intersect(features,rownames(GetAssayData(object, assay=assay.use, layer=layer.use)))
  features.negative <- intersect(unlist(lapply(strsplit(grep("^-",features,value=T),"^-"),`[`,2)),rownames(GetAssayData(object, assay=assay.use, layer=layer.use)))
  
  missing.features <- setdiff(GetAllSignatureGenes(features), rownames(GetAssayData(object, assay=assay.use, layer=layer.use)))
  ...
  if(length(features.positive)){
    scores.positive <- Matrix::colSums(rbind(GetAssayData(object, assay=assay.use, layer=layer.use)[features.positive,],0))/length(features) # Concatenate a line of zeros because colSums doesn't deal with one-line matrices.
  }else{
    scores.positive <- 0
  }
  if(length(features.negative)){
    scores.negative <- Matrix::colSums(rbind(GetAssayData(object, assay=assay.use, layer=layer.use)[features.negative,],0))/length(features)
  }else{
    scores.negative <- 0
  }
...
#' @param assay.use The assay within the Seurat object to use for signature scoring. If NA (default), it will use the "imputed" assay if it is present, or the "RNA" assay otherwise.
#' @param layer.use The layer to use within the assay to get data for signature scoring. By default, using the "data" assay.
#' @return Seurat object with additional metadata layers containing signature scores.
...
ScoreSignatures <- function(object, signature.list, assay.use=NA, layer.use="data"){
  for(i in names(signature.list)){
    object <- ScoreSignature(object, signature.list[[i]], i, assay.use=assay.use, layer.use=layer.use)
  }
  return(object)
}
